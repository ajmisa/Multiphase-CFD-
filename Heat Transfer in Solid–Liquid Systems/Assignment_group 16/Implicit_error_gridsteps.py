...
# Analytical solution
delta_tau_analytical = tau_max / timesteps
...
# Different gridsteps for numerical solution
gridsteps_list = [51, 101, 201, 401, 801]

# Error metrics storage
error_metrics = {"Gridsteps": [], "RMSE": [], "MAE": [], "L-infinity": []}

# Loop over different gridsteps
for gridsteps in gridsteps_list:
    ...
    # Store errors
    error_metrics["Gridsteps"].append(gridsteps)
    error_metrics["RMSE"].append(rmse)
    error_metrics["MAE"].append(mae)
    error_metrics["L-infinity"].append(l_infinity)

# Plot error metrics vs. gridsteps
plt.figure(figsize=(10, 6))
plt.plot(error_metrics["Gridsteps"], error_metrics["RMSE"], marker='o', label="RMSE")
plt.plot(error_metrics["Gridsteps"], error_metrics["MAE"], marker='s', label="MAE")
plt.plot(error_metrics["Gridsteps"], error_metrics["L-infinity"], marker='^', label="L-infinity")
plt.xlabel("Gridsteps")
plt.ylabel("Error")
plt.title("Error Metrics vs. Gridsteps (Fixed Timesteps = 10000)")
plt.xscale('log')  # Log scale for gridsteps
